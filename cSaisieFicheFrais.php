...
<?php
      ...
      <?php
            ...
            try {
              ...
              $statement = $idConnexion->prepare($req);
              $res = $statement->execute(array(':visiteur' => obtenirIdUserConnecte(), ':mois' => $mois));
              if ($res) {
                $lgEltForfait = $statement->fetch();
              ...
              echo "Erreur dans la construction de la liste des éléments forfaitisés: " . $e->getMessage();
            }
              ?>
          </fieldset>
      </div>
...
<?php
          ...
          try {
            ...
            $statement = $idConnexion->prepare($req);
            $res = $statement->execute(array(':mois' => $mois, ':visiteur' => obtenirIdUserConnecte()));

            $lgEltHorsForfait = $statement->fetch();
            ...
            echo "Erreur dans la construction de la liste des éléments hors forfait: " . $e->getMessage();
          }
?>
    </table>
      <form action="" method="post">
